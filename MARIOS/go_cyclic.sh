...
do
	...
	srun -t 10070 -n 1 -p shared --cpus-per-task=$cpus_per_task --error="cyclic_experiment${x}_error.txt" -o="\=cyclic_experiment${x}_output.txt" --mem-per-cpu=11gb  bash -c "python execute_cyclic.py '$x' $cpus_per_task '$size' '$model_type' '$activation'" & #-c 30 -N 1 
done
