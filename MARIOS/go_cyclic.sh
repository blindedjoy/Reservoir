...
do
	...
	srun -t 10070 -n 1 -p shared --cpus-per-task=$cpus_per_task --error="cluster_output/cyclic_experiment${x}_error.txt" -o="cluster_output/\=cyclic_experiment${x}_output.txt" --mem-per-cpu=11gb  bash -c "python execute_cyclic.py '$x' $cpus_per_task" & #-c 30 -N 1 
done
