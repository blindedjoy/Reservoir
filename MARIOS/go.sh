...
do
	#echo $x -p shared
	srun -t 6000 -n 1 -p shared --cpus-per-task=$cpus_per_task --error="new_experiment${x}_error.txt" -o="new_experiment${x}_output.txt" --mem-per-cpu=11gb  bash -c "python execute.py '$x' $cpus_per_task" & #-c 30 -N 1 
done
